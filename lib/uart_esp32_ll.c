...
void uart_init(uart_handle_t uart, uint32_t baudrate)
{
    ...
    uart_set_pin(uart, UART2_TX_PIN, UART2_RX_PIN,
                 UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
}

// IRAM_ATTR obligatorio si usás ESP_INTR_FLAG_IRAM

void IRAM_ATTR ISR_uart2(void *arg)
{
    uart_dev_t *n = &UART2;
    ...
    if (status & UART_INTR_TXFIFO_EMPTY) {
        ...
        while (tx_free--) {
            ...
            if (!c) {
                ...
                uart_disable_tx_intr(UART_NUM_2);       // análogo a USART_DisableInterrupts
                break;
            }
